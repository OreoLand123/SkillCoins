...
async def send_message_start(message):
    ...
    if await check_admin(str(message.from_user.id), message.from_user.username):
        ...
        await bot.send_message(message.from_user.id, texts["hello_admin"], reply_markup=keyboard.kb_mark_6)
...
async def send_message_update(message):
    if await check_admin(str(message.from_user.id), message.from_user.username):
        ...
        await bot.send_message(message.from_user.id, random.choice(mes).format(str(res[0])), reply_markup=keyboard.kb_mark_6)
        if res[1] != []:
            errors_mes = ''
            for err in res[1]:
                errors_mes += f"{err[2]} лишних логов: {err[0]} {err[1]}\n"
            await bot.send_message(message.from_user.id, random.choice(texts["errors"]).format(errors_mes), reply_markup=keyboard.kb_mark_6)
